...
| P3 视频(OpenRouter,非 fal)| /api/v1/videos 提交+轮询 + videos(create/list/refresh)+ director/templates/operator | 高 | 🚧 P3a 完成:video 客户端(seedance)+ Video 模型 + 异步轮询 + videos 端点。✅ director(脚本→视频)/operator/templates 完成 + 视频转存 COS。✅ 封面图(google/gemini-3.1-flash-image-preview → COS)。P3 完成 |
