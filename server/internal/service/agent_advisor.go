...
const (
	...
	advisorHistoryClip = 1600 // 保留足够的顾问上下文,仅防极长 Listing/解析产出撑爆会话
)
...
- 中文、口语化、结论先行,不摆套话。根据问题复杂度决定篇幅:简单问题直接回答;需要路线规划、方案比较、成本核算或风险分析时可以充分展开,用小标题、列表或表格帮助用户决策。不要为了凑短而省略关键前提,也不要重复同一结论。
- 结合会话里已有的往来(比如已选的品、已出的脚本、复盘结论)给建议,别让用户重复已经做过的事。
- 明确区分“已知事实”“经验估算”“仍需确认”;没有依据时不编数字、政策、网址、案例或平台能力。
- 用户附图时，先说明图片里确实看到了什么，再结合他的问题给建议。看不清的内容直接说需要补充；不能只凭图片推断商品规格、销量、成本、佣金或平台政策。
...
func (s *AgentService) runAdvisor(ctx context.Context, taskID, wsID uuid.UUID, input string, opts AgentCreateOpts, emit func(string)) (string, any, llm.Usage, error) {
	...
	if opts.MaterialID != nil {
		imageURL := s.materialImageURL(ctx, wsID, *opts.MaterialID)
		if imageURL == "" {
			return "", nil, llm.Usage{}, fmt.Errorf("找不到添加的图片,请重新上传")
		}
		imageURLs = []string{imageURL}
		user += "\n\n用户在本轮主动附加了一张图片。请直接查看图片，并结合用户问题回答。"
	}
	...
	var err error
	if len(imageURLs) > 0 {
		res, err = s.llm.ChatThreadVisionWithOptions(
			lctx, s.llm.AdvisorModel(), advisorSystem, thread, imageURLs, false,
			8000, optsForChat,
		)
	} else {
		res, err = s.llm.ChatThreadWithOptions(
			lctx, s.llm.AdvisorModel(), advisorSystem, thread, false,
			8000, // M3 的推理也计入 completion budget,留足空间避免复杂回答在正文中途截断
			optsForChat,
		)
	}
	if err != nil {
