...
// GetProductDetailMap 按 product_ids 批量取详情,返回 productID -> 完整 ProductDetail
// (封面原文 + 近 7/30/90 天窗口 + 累计权威值,一次调用全带回,调用方按需取用)。
// 商品榜(product/ranklist)不返回 cover/窗口,只能从 /echotik/product/detail 拿。
