...
// 额度按「整数条 8s 出片」反推(280 积分/条),对外才好讲清能出几条:PRO=20 条、TEAM 基线=40 条。
// 单价 ¥0.0355/积分(PRO ¥199/5600),满用毛利 30%。
//
// PRO 的额度上限是硬的:打平点 = ¥199/¥0.0249 ≈ 8000 积分(全烧出片时毛利归零),
// 5600 只用掉 70%。别做「加量不加价」——过 8000 就是白干。
...
// 不阻断出片。含 40 条 8s 出片(11200/280)。
// 基线必须跟着月费走:TEAM 不限量且超基线不阻断,月费是唯一的成本保底 —— 基线跑满的
// 视频成本(11200×¥0.0249≈¥279)必须低于月费(¥399),否则中度用户段会倒挂。
// 按 ¥399/11200 = ¥0.0356/积分标定,与 PRO 同价;TEAM 卖的是超出不断供 + 按量结算。
const TeamBaselineCredits = 11200
