...
type AgentTask struct {
	...
	// ConversationID 归属会话(多会话改造新增)。存量任务由启动时 BackfillConversations 回填:一条任务一个会话。
	ConversationID uuid.UUID  `gorm:"column:conversation_id;type:uuid;index:idx_task_conv_created" json:"conversationId"`
	Agent          string     `gorm:"not null;index:idx_task_ws_agent_created" json:"agent"` // ANALYST|DIRECTOR|LISTING|REVIEW
